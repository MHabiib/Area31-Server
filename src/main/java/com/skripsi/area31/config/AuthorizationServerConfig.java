...
@Configuration @EnableAuthorizationServer public class AuthorizationServerConfig
    ...
    @Override public void configure(ClientDetailsServiceConfigurer configurer) throws Exception {
        configurer.inMemory().withClient(FieldName.Constants.CLIENT_ID).secret(encoder().encode(FieldName.Constants.CLIENT_SECRET))
            .authorizedGrantTypes(FieldName.Constants.GRANT_TYPE_PASSWORD, FieldName.Constants.AUTHORIZATION_CODE, FieldName.Constants.REFRESH_TOKEN, FieldName.Constants.IMPLICIT)
            .scopes(FieldName.Constants.SCOPE_READ, FieldName.Constants.SCOPE_WRITE, FieldName.Constants.TRUST)
