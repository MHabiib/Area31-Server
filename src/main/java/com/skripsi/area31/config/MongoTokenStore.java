...
import com.skripsi.area31.model.mongo.MongoAccessToken;
import com.skripsi.area31.model.mongo.MongoRefreshToken;
...
@Component public class MongoTokenStore implements TokenStore {
    ...
    public void revokeToken(String email) {
        List<OAuth2AccessToken> tokens = findTokensByClientIdAndUserName(FieldName.Constants.CLIENT_ID, email);
        if (tokens.size() > 0) {
    ...
    private String extractTokenKey(String value) {
        if (Objects.isNull(value)) {
            ...
            try {
                ...
                throw new IllegalStateException(FieldName.Constants.MD_ALGORITHM_NOT_AVAILABLE);
            }
            try {
                byte[] e = digest.digest(value.getBytes(StandardCharsets.UTF_8.name()));
                return String.format("%032x", new BigInteger(1, e));
            } catch (UnsupportedEncodingException var4) {
                throw new IllegalStateException(FieldName.Constants.ENCODING_NO_AVAILABLE);
            }
        }
    }
}
